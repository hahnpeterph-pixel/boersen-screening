...
Laeuft auf Knopfdruck, nicht nachts. Schreibt:
  ...
  docs/puffer_je_tief.csv.gz eine Zeile JE TIEF - die Rohdaten (gepackt)
...
# Gepackt geschrieben: die Datei hat rund 30000 Zeilen und 158 Spalten und
# waegt ungepackt 15 MB. Jeder Lauf legte bisher einen neuen Blob dieser
# Groesse im Repo ab. Gepackt sind es 3 MB. Lesen in pandas geht ohne
# Zutun: pd.read_csv() erkennt die Endung .gz selbst.
CSV_ROH = DOCS / "puffer_je_tief.csv.gz"
...
# Die Strategie kauft Tiefs INNERHALB eines laengeren Aufwaertstrends.
# Bis 24.08.2026 beschrieb keine Spalte diese uebergeordnete Lage - alle
# vorhandenen Groessen (Position, Serienlaenge, Abstand, RSI) beschreiben
# die Korrektur selbst. Damit war die Kernannahme der Strategie nicht
# pruefbar. Drei Zahlen je Tief schliessen die Luecke; gruppiert wird
# bewusst nicht, die Rohspalten stehen in puffer_je_tief.csv.gz und lassen
...
def faelle_je_wert(ticker: str, df: pd.DataFrame) -> list[dict]:
    ...
    # EMA200 als Reihe. min_periods sorgt dafuer, dass die ersten 200 Tage
    # leer bleiben statt einen Wert vorzutaeuschen, der auf zwanzig Kerzen
    # beruht.
    ...
    for s in seqs:
        for pos, i in enumerate(s["tiefs"], start=1):
            ...
            # ── Uebergeordneter Trend am Bestaetigungstag ──────────────
            # Alle drei in ATR, damit sie ueber Werte hinweg vergleichbar
            # sind. Fehlt das noetige Fenster, bleibt die Zahl None und die
            # Spalte leer - kein Ersatzwert, der eine Aussage vortaeuscht.
            e_b = ema_lang[b]
            ema200_atr = (None if not np.isfinite(e_b)
                          else (einstieg_kurs - float(e_b)) / atr_i)
            ...
            # Korrektur, nicht die Lage nach der ersten Erholung.
            hoch250_atr = None
            if i + 1 >= HOCH_FENSTER:
                gipfel_jahr = float(hoch_w[i + 1 - HOCH_FENSTER:i + 1].max())
                hoch250_atr = (gipfel_jahr - float(tief_w[i])) / atr_i
